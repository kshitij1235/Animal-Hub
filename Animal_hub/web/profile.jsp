...
bg-indigo-600 hover:bg-indigo-700">x</a>
</div>
</div>
</div>
...
<div>
<a href="main_page.jsp" class="w-full flex items-center justify-center px-4 py-2 border
border-transparent rounded-md shadow-sm text-base font-medium text-white bg-indigo-600
hover:bg-indigo-700">x</a>
...
ResultSet rs = stmt.executeQuery("SELECT NAME,AGE FROM USERS WHERE EMAIL ='"+user+"'");
if (rs.next())
{
out.println("<dd class='mt-1 text-sm text-gray-900 sm:mt-0sm:col-span-2'>"+user+"</dd>");
out.println(" </div>");
...
out.println("</div>");
out.println(" <div class='bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4sm:px-6'>");

out.println("<dt class='text-sm font-medium text-gray-500'>Age</dt>");
out.println("<dd class='mt-1 text-sm text-gray-900 sm:mt-0sm:col-span-2'>"+rs.getString(2)+"</dd>");
out.println(" </div>");


}
%>
</div></dl></div></div></body></html>